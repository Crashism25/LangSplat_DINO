...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        if training_args.include_feature:
            # Check if we need to reinitialize language features
            need_reinit = (self._language_feature is None or
                          self._language_feature.shape[0] != self._xyz.shape[0] or
                          self._language_feature.shape[1] != training_args.language_feature_dim)

            if need_reinit:
                print(f"[INFO] Initializing language features with dimension {training_args.language_feature_dim}")
                if self._language_feature is not None:
                    print(f"[INFO] Previous dimension was {self._language_feature.shape[1]}")
                # 开始feature训练的时候，往模型中加入language feature参数
                language_feature = torch.zeros((self._xyz.shape[0], training_args.language_feature_dim), device="cuda")
                self._language_feature = nn.Parameter(language_feature.requires_grad_(True))
